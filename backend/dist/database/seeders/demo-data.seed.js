...
function img(keywords, width, height, alt) {
    imgLockCounter += 1;
    const tags = keywords
        .split(',')
...
async function seedCategories(model) {
    ...
    const defs = [
        ...
        {
            name: 'Wooden Chairs',
            description: 'Dining and accent chairs in solid wood, joined and finished by hand.',
            isFeatured: true,
...
async function seedBlogs(categoryModel, blogModel) {
    ...
    const docs = defs.map((d, i) => ({
        ...
        content: d.content,
        category: byName(d.category),
        featuredImage: img(blogKeywords[d.category] ?? 'wood furniture', 1200, 750, d.title),
...
async function seedProjects(model, categories) {
    ...
    const docs = defs.map((d, i) => ({
        ...
        category: byName(d.category),
        coverImage: img(projectKeywords[d.category] ?? 'wood furniture,interior', 1200, 900, d.title),
        images: [
            img(projectKeywords[d.category] ?? 'wood furniture,interior', 1200, 900, `${d.title} detail`),
        ],
        isFeatured: true,
        displayOrder: i,
...
async function seedGallery(model) {
    ...
    const docs = tags.map((t, i) => ({
        media: img(`${t.join(',')},wood furniture`, 1000, 1250, `Gallery image ${i + 1}`),
...
async function seedBanners(model) {
    ...
    const defs = [
        ...
        },
        {
            ...
            subtitle: 'No MDF, no veneer, no laminate shortcuts. Every piece is solid wood through and through.',
        },
        {
            placement: banner_schema_1.BannerPlacement.HERO,
            title: 'Made to Your Room, Not Off a Shelf',
            subtitle: 'Dining tables, wardrobes and cabinetry built to the exact dimensions you send us.',
        },
        {
            placement: banner_schema_1.BannerPlacement.HERO,
            title: 'Delivered Pan-India, With Installation Guidance',
            subtitle: 'Careful packing and delivery to your doorstep, wherever in India you are.',
        },
        {
    ...
    const bannerKeywords = {
        [banner_schema_1.BannerPlacement.HERO]: 'wood furniture,carpentry workshop',
        [banner_schema_1.BannerPlacement.CATEGORY]: 'wood furniture,showroom',
        [banner_schema_1.BannerPlacement.PRODUCT]: 'wood furniture,dining table',
        [banner_schema_1.BannerPlacement.BLOG]: 'woodworking,carpenter',
        [banner_schema_1.BannerPlacement.PROJECTS]: 'interior design,wood furniture',
        [banner_schema_1.BannerPlacement.ABOUT]: 'carpentry workshop,wood',
        [banner_schema_1.BannerPlacement.CONTACT]: 'carpentry workshop,wood',
    };
    ...
    const created = await model.create(docs);
    logger.log(`Seeded ${created.length} banners (${defs.filter((d) => d.placement === banner_schema_1.BannerPlacement.HERO).length} hero slides, one banner per other placement).`);
}
...
async function seedAboutPage(model) {
    ...
    await model.create({
        ...
        storyImage: img('carpenter,woodworking', 1200, 900, 'Woodivo craftsman at work'),
