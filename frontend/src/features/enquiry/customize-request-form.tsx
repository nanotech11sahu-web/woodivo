...
interface CustomizeRequestFormProps {
  /**
   * Set only when this form was reached from a product's "Customize this"
   * link (`/customize?product=<slug>`) — carried through to the Enquiry
   * as `interestedProduct` so the CMS inbox still shows which piece the
   * request started from. Both are optional: most visits to this page
   * come from the nav link with no product in mind at all, and the
   * request/showcase below it work exactly the same either way.
   */
  productSlug?: string;
  productName?: string;
}
...
/**
 * The standalone "Customize" request form — lives on its own page
 * (`/customize`) rather than embedded per product-detail page. Lets a
 * visitor attach up to 4 reference photos of what they want (a different
 * wood tone, a size tweak, an inlay pattern) plus a description. Submits
 * as an Enquiry with source: 'custom_order', same as before; the only
 * change from the old per-product version is that `interestedProduct` is
 * now optional context instead of a required prop.
 */
export function CustomizeRequestForm({ productSlug, productName }: CustomizeRequestFormProps) {
      ...
      <div className="rounded-[var(--radius-card)] border border-border-warm bg-white p-6 sm:p-8 lg:col-span-3">
        {submitted ? (
          ...
          <form onSubmit={(event) => void onSubmit(event)} className="flex flex-col gap-4" noValidate>
            {productName ? (
              <p className="rounded-[var(--radius-card)] bg-ivory-deep px-4 py-2.5 text-sm text-charcoal-soft">
                Customizing based on <span className="font-semibold text-teak">{productName}</span>
              </p>
            ) : null}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="e.g. A teak dining table for 10, in the style of our Heritage Teak Dining Table"
