...
interface ProductFaqEditorProps {
  ...
  onChange: (faqs: ProductFaqItem[]) => void;
}

/** Repeatable question/answer rows for a product's FAQ block — feeds both
 * the on-page FAQ section and the `FAQPage` JSON-LD on the frontend.
 * Mirrors `BlogFaqEditor` (features/blogs/blog-faq-editor.tsx). */
export function ProductFaqEditor({ value, onChange }: ProductFaqEditorProps) {
  function addRow() {
    onChange([...value, { question: '', answer: '' }]);
  }

  function updateRow(index: number, patch: Partial<ProductFaqItem>) {
